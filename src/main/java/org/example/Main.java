...
// Первая задача
        ...
        System.out.println(personAge >= 18 ? "Человек совершеннолетний" : "Человеку меньше 18, он не достиг совершеннолетия, нужно немного подождать"); //Тернарный оператор на определение возраста
// Вторая задача
        byte outsideWeather = 7; // Указываем температуру на улице
        if (outsideWeather <= 5) System.out.println("На улице - " + outsideWeather + " , нужно надеть шапку.");
...
// Третья задача
        ...
        System.out.println(actualSpeed > speedLimit ? "Скорость превышена" : "Превышения скорости нет");
//Четвертая задача
        ...
        } else if (actualAge > 24)//Возраст хождения на работу
        {
            ...
            System.out.println("Человек еще слишком маленький, малышам надо сидеть дома с мамой.");
        }//Вывод результата, если возраст меньше 2
// Пятая задача
...
//Седьмая задача
        ...
        int two=3;
        int three=2;
        if (one>two && one>three) {System.out.println("Первое значение максимальное");}
            else if (one<two && two>three) {System.out.println("Второе значение максимальное");}
            else if (one<three && two<three) {System.out.println("Третье значение максимальное");}
